...
if_pred_y = [1 if e == -1 else 0 for e in if_y_pred_test]
conf_matrix = confusion_matrix(y_test, if_pred_y)
cnf_matrix = metrics.confusion_matrix(y_test, if_pred_y)
tn, fp, fn, tp = confusion_matrix(y_test, if_pred_y).ravel()
if_specificity = tn / (tn+fp)
if_sensitivity  = tp / (fn + tp)
roc_if=roc_auc_score(y_test, if_pred_y)

print('for Isolation forest:')
print("Accuracy:",metrics.accuracy_score(y_test, if_pred_y))
print("Precision:",metrics.precision_score(y_test, if_pred_y))
print("Recall:",metrics.recall_score(y_test, if_pred_y))
print('specificity:', if_specificity)
print('sensitivity:', if_sensitivity)
print('AUC: ',roc_if)
...
clf = LocalOutlierFactor(n_neighbors=20, contamination=.03)
lof_y_pred_train = clf.fit_predict(x_train)
lof_y_pred_test = clf.fit_predict(x_test)

lofs_index = where(lof_y_pred_test==-1)
print(lofs_index)

pred_y = [1 if e == -1 else 0 for e in lof_y_pred_test]
conf_matrix = confusion_matrix(y_test, pred_y)
cnf_matrix = metrics.confusion_matrix(y_test, pred_y)
tn, fp, fn, tp = confusion_matrix(y_test, pred_y).ravel()
print(tn, fp, fn, tp)
lof_specificity = tn / (tn+fp)
lof_sensitivity  = tp / (fn + tp)
roc_lf=roc_auc_score(y_test, pred_y)
...
print('specificity:', lof_specificity)
print('sensitivity:', lof_sensitivity)
print('AUC: ',roc_lf)
...
#set height
bars1 = [metrics.accuracy_score(y_test, predictions),metrics.accuracy_score(y_test, svm_pred),rf_a, 0.8169014084507042, 0.9976525821596244, metrics.accuracy_score(y_test, if_pred_y), metrics.accuracy_score(y_test, pred_y)   ]
bars2 = [specificity, s_specificity, rf_specificity, 0.8020565552699229, 1,if_specificity,lof_specificity  ]
bars3 = [sensitivity, s_sensitivity, rf_sensitivity,0.972972972972973,0.972972972972973 , if_sensitivity, lof_sensitivity ]
bars4 = [roc_lr, roc_svm, roc_rf, 0.8875147641214479, 0.9864864864864865,roc_if,roc_lf  ]
...
plt.xticks([r + barWidth for r in range(len(bars1))], ['Logistic Regression','SVM','Random Forrest', 'SVM One Class', 'Autoencoder', 'Isolation Forest', 'LOF'], rotation = 'vertical')
plt.show

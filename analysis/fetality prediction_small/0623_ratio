...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 25 14:58:56 2020
...
def perpareTrainData(df):
    ...
    df_a_nontest = df_a[:1013]
    df_a_test = df_a[1013:]

    df_d_nontest = df_d[:86]
    df_d_test = df_d[86:]

    nontest_df = pd.concat([df_a_nontest, df_d_nontest], axis=0)
    test_df = pd.concat([df_a_test, df_d_test], axis=0)

    df_a_train = df_a_nontest[:709]
    df_a_valid = df_a_nontest[709:]

    df_d_train = df_d_nontest[:60]
    df_d_valid = df_d_nontest[60:]
